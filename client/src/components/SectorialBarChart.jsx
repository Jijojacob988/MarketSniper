...
const SectorialBarChart = ({
  ...
  const sortedData = useMemo(() => {
    return [...cleanedData].sort((a, b) => b.value - a.value);
  }, [cleanedData]);
  ...
  const scrollToTable = (sectorSymbol) => {
    const section = document.getElementById(sectorSymbol);
    if (section) {
      section.scrollIntoView({ behavior: "smooth", block: "center" });
      section.classList.add("highlight-glow");
      setTimeout(() => section.classList.remove("highlight-glow"), 1500);
    }
  };
    ...
    <div className="bg-[#181818] border border-[#2a2a2a] rounded-xl p-3 sm:p-4 shadow-md relative">
          ...
          >
            {visibleData.map((entry, idx) => (
              <Cell
                ...
                onClick={() => scrollToTable(entry.symbol)}
              />
            ))}
          </Bar>
        </BarChart>
      ...
      <style jsx>{`
        ...
        @keyframes glowEffect {
          ...
          50% {
            box-shadow: 0 0 20px 5px #facc1577;
          }
          100% {
            box-shadow: 0 0 0px #facc15;
          }
...
export default SectorialBarChart;
